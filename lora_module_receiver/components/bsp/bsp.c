...
#define UART_TX_PIN        17  /* ESP32-S3 TX Pin */
#define UART_RX_PIN        18  /* ESP32-S3 RX Pin */
...
int32_t bsp_uart_read(uint8_t *buffer, uint16_t max_length) {
    int length = uart_read_bytes(UART_PORT_NUM, buffer, max_length - 1, pdMS_TO_TICKS(50));
