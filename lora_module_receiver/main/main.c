...
void parse_and_print_payload(const char *raw_packet) {
    ...
    if (message_len >= sizeof(decoded_message)) message_len = sizeof(decoded_message) - 1; 
    ...
    ESP_LOGW("LORA_DATA", "=======================================");
    ESP_LOGW("LORA_DATA", "CROP SENSOR DATA: %s", decoded_message);
    ESP_LOGW("LORA_DATA", "=======================================\n");
}

void app_main(void) {
    ESP_LOGI(TAG, "Initializing LoRa Gateway (ESP32-S3) - SF10 Mode");
    ...
    if (WIO_E5_Driver.Ping(&radio) == 0) {
        ESP_LOGI(TAG, "GATEWAY RADIO IS ALIVE AND PINGED SUCCESSFULLY!");
    } else {
        ESP_LOGE(TAG, "FATAL: GATEWAY RADIO IS DEAD OR WIRES ARE LOOSE!");
    }
    ...
    const TickType_t TIMEOUT_TICKS = pdMS_TO_TICKS(45000); /* 45 seconds */
    ...
    while (1) {
        ...
        if (bytes > 0) {
            /* Check for a valid LoRa RX prefix */
        ...
        if ((xTaskGetTickCount() - last_packet_time) > TIMEOUT_TICKS) {
            ESP_LOGE(TAG, "WATCHDOG TRIGGERED: No data for 45 seconds. Re-arming Radio!");
            WIO_E5_Driver.StartReceive(&radio);
            last_packet_time = xTaskGetTickCount();
        }
