...
int32_t BSP_I2C_Init(void) {
    ...
    if (i2c_new_master_bus(&bus_config, &bus_handle) != ESP_OK) return SHT31_ERROR;
    ...
    if (i2c_master_bus_add_device(bus_handle, &dev_config, &dev_handle) != ESP_OK) return SHT31_ERROR;
    return SHT31_OK;
}

int32_t BSP_I2C_DeInit(void) {
    if (dev_handle != NULL) i2c_master_bus_rm_device(dev_handle);
    if (bus_handle != NULL) i2c_del_master_bus(bus_handle);
    return SHT31_OK;
}
...
static int32_t BSP_ADC_DeInit(void) {
    if (adc1_handle != NULL) adc_oneshot_del_unit(adc1_handle);
    return 0;
}

static int32_t BSP_ADC_Read(uint32_t *value) {
...
static SoilMoisture_IO_t Soil_IO = {
    BSP_ADC_Init,
    BSP_ADC_DeInit,
    BSP_ADC_Read
};

/* --- Your Custom Soil Wrapper API --- */
